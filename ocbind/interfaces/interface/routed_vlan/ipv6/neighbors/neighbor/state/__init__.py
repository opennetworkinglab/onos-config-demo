...
interface
  ...
  def _get_is_router(self):
    """
    Getter method for is_router, mapped from YANG variable /interfaces/interface/routed_vlan/ipv6/neighbors/neighbor/state/is_router (empty)
  ...
  def _set_is_router(self, v, load=False):
    """
    Setter method for is_router, mapped from YANG variable /interfaces/interface/routed_vlan/ipv6/neighbors/neighbor/state/is_router (empty)
    If this variable is read-only (config: false) in the
    ...
    if hasattr(v, "_utype"):
      v = v._utype(v)
    try:
      t = YANGDynClass(v,base=YANGBool, is_leaf=True, yang_name="is-router", parent=self, path_helper=self._path_helper, extmethods=self._extmethods, register_paths=True, namespace='http://openconfig.net/yang/interfaces/ip', defining_module='openconfig-if-ip', yang_type='empty', is_config=False)
    except (TypeError, ValueError):
      raise ValueError({
